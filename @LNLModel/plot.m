function plot(self, param, smooth_firing_rate)
  ...
  hd_vector = 2*pi/self.bins.head_direction/2:2*pi/self.bins.head_direction:2*pi - 2*pi/self.bins.head_direction/2;
  theta_vector = hd_vector;
  speed_vector = 2.5:50/self.bins.speed:47.5;
  ...
  pos_param = param_full_model(1:self.bins.position^2);
  hd_param = param_full_model(self.bins.position^2+1:self.bins.position^2+self.bins.head_direction);
  speed_param = param_full_model(self.bins.position^2+self.bins.head_direction+1:self.bins.position^2+self.bins.head_direction+self.bins.speed);
  theta_param = param_full_model(numel(param_full_model)-self.bins.theta+1:numel(param_full_model));
