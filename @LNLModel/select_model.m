...
function selected_model = select_model(self, testFit)
  ...
  if p_llh_12 < self.alpha % double model is sig. better
      if p_llh_23 < self.alpha  % triple model is sig. better
          if p_llh_34 < self.alpha % full model is sig. better
  ...
  pval_baseline = signrank(LLH_values(:, selected_model), self.baseline, 'tail', 'right');

  if pval_baseline > self.alpha
      % no selected model, none are significant
      corelib.verb(self.verbosity, 'WARN', 'no significant model found')
      selected_model = NaN;
  end
